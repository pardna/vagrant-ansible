...
if ( !class_exists( 'PT_CV_Html' ) ) {
	...
	class PT_CV_Html {
		...
		static function grid_item_wrap( $content_item ) {
			$class = PT_CV_PREFIX . 'ifield z-depth-1';
			...
			switch ( $fargs[ 'content' ][ 'show' ] ) {
				case 'excerpt':
					...
					if ( apply_filters( PT_CV_PREFIX_ . 'field_content_readmore_enable', 1, $fargs[ 'content' ] ) ) {
						...
						$btn_class		 = PT_CV_PREFIX . 'readmore waves-effect waves-light teal ' . apply_filters( PT_CV_PREFIX_ . 'field_content_readmore_class', 'btn btn-success', $fargs );
